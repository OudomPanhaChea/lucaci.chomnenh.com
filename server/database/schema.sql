...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  `base_unit`      VARCHAR(30)   NOT NULL DEFAULT 'pcs', -- what one stock count means: pcs, tubes, bottles ...
...
-- ── Partner bonuses (manual rewards, record + paper only) ──────────────────
-- Computed from a partner client's fully paid invoices in a period; the user
-- picks WHICH paid invoices count (all by default). Both reward levels are
-- optional: the item level = per-invoice item lines that reached a piece
-- threshold, the invoice level ("level1" columns) = pct of the selected
-- invoices' grand total or a fixed amount. Never touches the payments ledger
-- or credit_balance.
...
CREATE TABLE IF NOT EXISTS `bonus_items` (
  ...
  `pieces`         INT(11)       NOT NULL DEFAULT 0,  -- qty x unit_factor in that invoice (base units)
  `qty_desc`       VARCHAR(190)  DEFAULT NULL,        -- human snapshot: "10 × Box of 8 + 5 tubes"
  `line_total`     DECIMAL(10,2) NOT NULL DEFAULT 0.00,
